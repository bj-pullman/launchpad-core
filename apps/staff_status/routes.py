...
def index():
    ...
    if len(departments) == 1:
        return redirect(url_for("staff_status.department_overview", department_name=departments[0]["department_name"]))

    return render_template("staff_status/index.html", departments=departments)
...
def board_public_data(token: str):
    department = get_department_by_board_token(token)
    if not department:
        return jsonify({"ok": False, "error": "Invalid board token."}), 404
